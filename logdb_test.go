...
func TestNoConcurrentOpen(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "no_concurrent_open", chunkSize)
			_, lockerror := assertOpenError(t, false, "no_concurrent_open").(*LockError)
			assert.True(t, lockerror, "expected lock error")

			assertClose(t, db)
			db2 := assertOpen(t, dbType, false, "no_concurrent_open", chunkSize)
			assertClose(t, db2)
		}()
	}
}
...
func TestCorruptOldest(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "corrupt_oldest", chunkSize)
			...
			db2 := assertOpen(t, dbType, false, "corrupt_oldest", chunkSize)
...
func TestNoEmptyNonfinalChunk(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "no_empty_nonfinal_chunk", chunkSize)
			filldb(t, db, numEntries)
			assertClose(t, db)
...
func TestZeroSizeFinalChunk(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "zero_size_final_chunk", chunkSize)
			assertClose(t, db)
			...
			assertClose(t, assertOpen(t, dbType, false, "zero_size_final_chunk", chunkSize))
		}()
	}
}
...
func TestNoOpenZeroSizeNonfinalChunk(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "no_open_zero_size_nonfinal_chunk", chunkSize)
			filldb(t, db, numEntries)
			assertClose(t, db)
...
func TestMissingMetaFinalChunk(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "missing_meta_final_chunk", chunkSize)
			assertClose(t, db)
			...
			assertClose(t, assertOpen(t, dbType, false, "missing_meta_final_chunk", chunkSize))
...
func TestNoOpenMissingMetaNonfinalChunk(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "no_open_missing_meta_nonfinal_chunk", chunkSize)
			filldb(t, db, numEntries)
			assertClose(t, db)
...
func TestGap(t *testing.T) {
	for dbName, dbType := range dbTypes {
		t.Logf("Database: %s\n", dbName)
		func() {
			db := assertOpen(t, dbType, true, "gap", chunkSize)
			filldb(t, db, numEntries)
			assertClose(t, db)
			...
			assertClose(t, assertOpen(t, dbType, false, "gap", chunkSize))
